...
function PoolRow({ reserves }: { reserves: Array<ReserveWithMetadataType> }) {
  ...
  if (!reserves.length)
    return (
      <Text mt={1} variant='label' color='secondary'>
        -
      </Text>
    );
  return (
...
export default function Nav({ onClose }: { onClose?: () => void }) {
  ...
  const sortedVisiblePools = visiblePools.sort((a, b) => {
    ...
    const poolADeprecated = Number(
      Boolean(poolA.reserves.every((r) => r.disabled)),
    );
    const poolBDeprecated = Number(
      Boolean(poolB.reserves.every((r) => r.disabled)),
    );

    if (poolADeprecated === poolBDeprecated) {
      return poolA.totalSupplyUsd.isGreaterThan(poolB.totalSupplyUsd) ? -1 : 1;
    }
              ...
              onChange={(e) => {
                ...
                const filteredPools = Object.values(pools).filter((market) => {
                  ...
                  searchTerms.forEach((term) => {
                    const similarities = stringComparison.lcs.sortMatch(
                      term,
                      keywords,
                    );
