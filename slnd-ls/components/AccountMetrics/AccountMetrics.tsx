  ...
  return (
    <Box p={4}>
      <Flex >
        <Metric
          label='Net value'
          value={obligation ? `${formatUsd(obligation.netAccountValue.toString())}` : '-'}
          tooltip='The value of your account calculated as (supply balance - borrow balance).'
        />
        <Spacer/>
        <Metric
          label='Borrow utilization'
          value={obligation ? `${formatPercent(obligation.borrowUtilization.toString())}` : '-'}
    ...
    <Metric
        ...
        value={obligation ? `${formatUsd(obligation.totalBorrowValue.toString())}` : '-'}
        tooltip='Borrow balance is the sum of all assets borrowed.'
    />
    <Spacer/>
    <Metric
        label='Supply balance'
        value={obligation ? `${formatUsd(obligation.totalSupplyValue.toString())}` : '-'}
    ...
    />
        ...
        <Metric
            row
          label='Borrow limit'
          value={obligation ? `${formatUsd(obligation.borrowLimit.toString())}` : '-'}
          tooltip={
            <>
              ...
              <br />
              <br />
              (Currently {obligation ? formatPercent(obligation.borrowLimitFactor.toString()) : '-'} of supply
              balance).
            </>
          }
        />
        ...
        row
          ...
          value={obligation ? `${formatUsd(obligation.liquidationThreshold.toString())}` : '-'}
          tooltip={
            <>
