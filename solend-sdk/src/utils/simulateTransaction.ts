...
export async function simulateRefreshObligation(
  ...
  const obligationAccount = await connection.getAccountInfo(obligationId);
  if (obligationAccount === null) {
    return null;
  }

  const obligation = parseObligation(obligationId, obligationAccount)!;
  ...
  const simulatedObligation = parseObligation(obligationId, obligationAccount)!;
