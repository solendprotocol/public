...
class SolendActionCore {
    ...
    static async initialize(pool, reserve, action, amount, publicKey, connection, environment = "production", customObligationAddress, hostAta, customObligationSeed) {
        const seed = customObligationSeed ?? pool.address.slice(0, 32);
        const programId = (0, constants_2.getProgramId)(environment);
        const obligationAddress = customObligationAddress ??
        ...
        const userTokenAccountAddress = await (0, spl_token_1.getAssociatedTokenAddress)(new web3_js_1.PublicKey(reserve.mintAddress), publicKey, true);
        const userCollateralAccountAddress = await (0, spl_token_1.getAssociatedTokenAddress)(new web3_js_1.PublicKey(reserve.cTokenMint), publicKey, true);
    ...
    static async buildForgiveTxns(pool, reserve, connection, amount, publicKey, obligationAddress, environment = "production") {
        const axn = await SolendActionCore.initialize(pool, reserve, "deposit", new bn_js_1.default(amount), publicKey, connection, environment, obligationAddress);
        await axn.addSupportIxs("forgive");
        await axn.addForgiveIx();
        return axn;
    }
    static async buildDepositTxns(pool, reserve, connection, amount, publicKey, environment = "production", obligationAddress, obligationSeed) {
        const axn = await SolendActionCore.initialize(pool, reserve, "deposit", new bn_js_1.default(amount), publicKey, connection, environment, obligationAddress, undefined, obligationSeed);
        await axn.addSupportIxs("deposit");
        await axn.addDepositIx();
        return axn;
    }
    static async buildBorrowTxns(pool, reserve, connection, amount, publicKey, environment = "production", hostAta) {
        const axn = await SolendActionCore.initialize(pool, reserve, "borrow", new bn_js_1.default(amount), publicKey, connection, environment, undefined, hostAta);
        await axn.addSupportIxs("borrow");
        await axn.addBorrowIx();
        return axn;
    }
    static async buildDepositReserveLiquidityTxns(pool, reserve, connection, amount, publicKey, environment = "production") {
        const axn = await SolendActionCore.initialize(pool, reserve, "mint", new bn_js_1.default(amount), publicKey, connection, environment);
        await axn.addSupportIxs("mint");
        await axn.addDepositReserveLiquidityIx();
        return axn;
    }
    static async buildRedeemReserveCollateralTxns(pool, reserve, connection, amount, publicKey, environment = "production") {
        const axn = await SolendActionCore.initialize(pool, reserve, "redeem", new bn_js_1.default(amount), publicKey, connection, environment);
        await axn.addSupportIxs("redeem");
        await axn.addRedeemReserveCollateralIx();
        return axn;
    }
    static async buildDepositObligationCollateralTxns(pool, reserve, connection, amount, publicKey, environment = "production") {
        const axn = await SolendActionCore.initialize(pool, reserve, "depositCollateral", new bn_js_1.default(amount), publicKey, connection, environment);
        await axn.addSupportIxs("depositCollateral");
        await axn.addDepositObligationCollateralIx();
        return axn;
    }
    static async buildWithdrawCollateralTxns(pool, reserve, connection, amount, publicKey, environment = "production") {
        const axn = await SolendActionCore.initialize(pool, reserve, "withdrawCollateral", new bn_js_1.default(amount), publicKey, connection, environment);
        await axn.addSupportIxs("withdrawCollateral");
        await axn.addWithdrawIx();
        return axn;
    }
    static async buildWithdrawTxns(pool, reserve, connection, amount, publicKey, environment = "production") {
        const axn = await SolendActionCore.initialize(pool, reserve, "withdraw", new bn_js_1.default(amount), publicKey, connection, environment);
        await axn.addSupportIxs("withdraw");
        await axn.addWithdrawIx();
        return axn;
    }
    static async buildRepayTxns(pool, reserve, connection, amount, publicKey, environment = "production") {
        const axn = await SolendActionCore.initialize(pool, reserve, "repay", new bn_js_1.default(amount), publicKey, connection, environment);
    ...
    async sendTransactions(sendTransaction, preCallback, lendingCallback, postCallback) {
    ...
    async addWithdrawObligationCollateralIx() {
        const buffer = await this.connection.getAccountInfo(new web3_js_1.PublicKey(this.reserve.address), "processed");
        if (!buffer) {
            throw Error(`Unable to fetch reserve data for ${this.reserve.address}`);
        }
        const parsedData = (0, reserve_1.parseReserve)(new web3_js_1.PublicKey(this.reserve.address), buffer)?.info;
        if (!parsedData) {
        ...
        const totalLiquidityWads = parsedData.liquidity.availableAmount.mul(new bn_js_1.default(constants_2.WAD));
        ...
        const cTokenExchangeRate = new bignumber_js_1.default(totalDepositsWads.toString())
            ...
            .div(constants_2.WAD);
        this.lendingIxs.push((0, instructions_1.withdrawObligationCollateralInstruction)(this.amount.eq(new bn_js_1.default(constants_2.U64_MAX))
            ? this.amount
    ...
    async addWithdrawIx() {
        ...
        const cTokenExchangeRate = new bignumber_js_1.default(totalDepositsWads.toString())
            .div(parsedData.collateral.mintTotalSupply.toString())
            .div(constants_2.WAD);
        this.lendingIxs.push((0, instructions_1.withdrawObligationCollateralAndRedeemReserveLiquidity)(this.amount.eq(new bn_js_1.default(constants_2.U64_MAX))
            ? this.amount
            : new bn_js_1.default(new bignumber_js_1.default(this.amount.toString())
                .dividedBy(cTokenExchangeRate)
                .integerValue(bignumber_js_1.default.ROUND_FLOOR)
